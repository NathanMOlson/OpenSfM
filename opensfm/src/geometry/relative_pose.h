...
template <class IT>
struct RelativePoseRotationCost {
  RelativePoseRotationCost(IT begin, IT end,
                           const Eigen::Matrix<double, 3, 1>& translation)
      : begin_(begin), end_(end), translation_(translation) {
    std::srand(42);
    const int count = end_ - begin_;
    for (int i = 0; i < MAX_ERRORS; ++i) {
      // Note that float(RAND_MAX) cannot be exactly represented as a float. We
  ...
  bool operator()(const T* const parameters, T* residuals) const {
    ...
    Eigen::Matrix<T, 2, 3> centers;
    centers.row(0) = Eigen::Matrix<T, 3, 1>::Zero();
    centers(1, 0) = T(translation_(0));
    centers(1, 1) = T(translation_(1));
    centers(1, 2) = T(translation_(2));
    Eigen::Matrix<T, 3, 1> some_tmp = Eigen::Matrix<T, 3, 1>::Zero();
    ...
    for (int i = 0; i < MAX_ERRORS; ++i) {
      ...
      // R*(y-t)
      const Eigen::Matrix<T, 3, 1> projected_x = point.second.normalized();
      const Eigen::Matrix<T, 3, 1> y_centered = point.second - translation_;
      ceres::AngleAxisRotatePoint(rotation.data(), y_centered.data(),
                                  some_tmp.data());
      const Eigen::Matrix<T, 3, 1> projected_y = some_tmp.normalized();
      residuals[i] = 1.0 - ((projected_x.dot(x) + projected_y.dot(y)) * T(0.5));
    }
    residuals[MAX_ERRORS] = T(0.0);
...
template <class IT>
Eigen::Matrix<double, 3, 4> RelativePoseRotationRefinement(
    const Eigen::Matrix<double, 3, 4>& relative_pose, IT begin, IT end,
    int iterations) {
  ...
  using RelativePoseFunction = ceres::TinySolverAutoDiffFunction<
      RelativePoseRotationCost<IT>,
      RelativePoseRotationCost<IT>::MAX_ERRORS + 1, 3>;
  RelativePoseRotationCost<IT> cost(begin, end, translation);
  RelativePoseFunction f(cost);
  ...
  ceres::AngleAxisToRotationMatrix(
      parameters.data(), relative_pose_result.block<3, 3>(0, 0).data());
  relative_pose_result.col(3) =
      -relative_pose_result.block<3, 3>(0, 0) * translation;
  return relative_pose_result;
}
namespace geometry {
Eigen::Matrix<double, 3, 4> RelativePoseFromEssential(
